...
public class DetectorService extends Service {
    ...
    private static float anger;
    private static float surprise;
    private static float joy;
    private static float sad;
    private static float disgust;
    private static float fear;
    ...
    private static class DetectionHandler extends Handler {
        ...
        private static class DetectorListener implements FrameDetector.ImageListener, FrameDetector.FaceListener {
            ...
            public void onFaceDetectionStarted() {
                Toast.makeText(context, R.string.face_found, Toast.LENGTH_SHORT).show();
            }

            @Override
            public void onFaceDetectionStopped() {
                Toast.makeText(context, R.string.face_lost, Toast.LENGTH_SHORT).show();
            }
            ...
            public void onImageResults(List<Face> faces, Frame frame, float v) {
                if (faces.size() > 0) {
                    ...
                    Log.d(DemoApplication.LOG_TAG, "Anger = " + Float.toString(face.emotions.getAnger()));
                    Log.d(DemoApplication.LOG_TAG, "Surprise = " + Float.toString(face.emotions.getSurprise()));
                    Log.d(DemoApplication.LOG_TAG, "Joy = " + Float.toString(face.emotions.getJoy()));
                    Log.d(DemoApplication.LOG_TAG, "Sad = " + Float.toString(face.emotions.getSadness()));
                    Log.d(DemoApplication.LOG_TAG, "Disgust = " + Float.toString(face.emotions.getDisgust()));
                    Log.d(DemoApplication.LOG_TAG, "Fear = " + Float.toString(face.emotions.getFear()));
                    anger = face.emotions.getAnger();
                    surprise = face.emotions.getSurprise();
                    joy = face.emotions.getJoy();
                    sad = face.emotions.getSadness();
    ...
    public static float getsurprise() {
        return surprise;
    }

    public static float getjoy() {
        return joy;
    }

    public static float getsad() {
        return sad;
    }

    public static float getdisgust() {
        return disgust;
    }
